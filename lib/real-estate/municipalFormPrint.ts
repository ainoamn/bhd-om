...
export function openMunicipalFormPrintWindow(
  ...
  const title = ar ? 'استمارة بلدية — عقد إيجار' : 'Municipal form — tenancy contract';
  ...
  const rows = [
    fieldRow(ar ? 'اسم الشركة / المؤجر' : 'Company / landlord', company?.nameAr || company?.nameEn || '—', ar),
    fieldRow(ar ? 'المالك' : 'Owner', unit.ownerNames || '—', ar),
    fieldRow(ar ? 'رقم استمارة بلدية' : 'Municipal form no.', values.municipalFormNo, ar),
    fieldRow(ar ? 'رقم عقد بلدي' : 'Municipal contract no.', values.municipalContractNo, ar),
    fieldRow(ar ? 'رقم العقد الداخلي' : 'Internal agreement no.', values.agreementNo, ar),
...
<body>
  ...
  <table>${rows}</table>
  <p style="margin-top:20px;font-size:8pt;opacity:0.7;">${escapeHtml(
    ar
      ? 'نسخة للمراجعة والطباعة — للنموذج البلدي الرسمي المعتمد راجع النظام التشغيلي.'
      : 'Review copy for printing — use the operational system for the official municipal form.'
  )}</p>
</body>
</html>`);
  win.document.close();
