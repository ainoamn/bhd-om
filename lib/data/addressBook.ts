...
/** إيجاد جميع جهات الاتصال المكررة — cache محلي؛ للعرض من الخادم استخدم findDuplicateContactGroupsFromList(contacts) */
export function findDuplicateContactGroups(): Contact[][] {
  return findDuplicateContactGroupsFromList(getStored());
}

/** دمج جهات اتصال مكررة — cache محلي؛ يُفضّل POST /api/admin/address-book/merge-duplicates */
export function mergeDuplicateContacts(contactIds: string[]): Contact | null {
  ...
  const result = mergeContactGroup(toMerge);
  if (!result) return null;
  const { merged, removeIds } = result;
  const removeSet = new Set(removeIds);
  const newList = list.filter((c) => !removeSet.has(c.id));
  const idx = newList.findIndex((c) => c.id === merged.id);
  if (idx >= 0) newList[idx] = merged;
  else newList.unshift(merged);
  save(newList);
  void syncContactToAddressBookApi(merged);
  return merged;
}
...
/** استيراد جهات الاتصال من CSV - يرجع عدد السجلات المستوردة (cache محلي — يُفضّل API الخادم) */
export function importContactsFromCsv(csvText: string): number {
  ...
  for (const row of parsed) {
    const dups = findDuplicateContactFields(row.phone, row.civilId, row.passportNumber);
    if (dups.phone || dups.civilId || dups.passportNumber) continue;
    list.push({ ...row, id: generateId() });
    count++;
  }
