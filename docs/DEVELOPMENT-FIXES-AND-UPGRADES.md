...
- إكمال فصل واجهة العقود نهائياً عن `localStorage` — **2026-05-31:** `resolveContractFromBooking`؛ صفحات admin/bookings/contracts/my-bookings + `contracts/[id]` server-first؛ `updateContract` → PATCH؛ `fetchContractsFromServer` لا fallback محلي؛ اعتماد من قائمة admin/contracts يعمل على عقود الخادم.
...
### أولوية متوسطة
- تقليل الـ fallback المحلي في `address-book` — **2026-05-31:** عمليات bulk من API؛ العرض من الخادم فقط؛ archive/restore/create-accounts بدون `getAllContacts`. **تابع 29:** create/update/archive عبر `lib/client/addressBookServerApi.ts`. **تابع 31–32:** `my-account` و `properties/.../book` عبر `linked-contact` API.
