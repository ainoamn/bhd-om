...
- **إصلاح السجل:** إزالة تكرار الأرشيف بنفس رقم الاتفاق؛ إظهار العقد الحالي المحفوظ مع الشيكات؛ استكمال العقود الناقصة من سجل التجديد؛ تفاصيل أغنى عند الضغط على الحالة؛ فلاتر (عقود/تجديد/حجوزات…).
- **إصلاح الصفحة:** شريط ملخص سريع + ترتيب مرقّم (وحدة → مستأجر → عقد → سجل عقود → مبالغ → مستندات → محاسبة…).
- **BOOT_REV:** `2026-07-21-unit-details-history` — Ctrl+F5.

### جلسة 2026-07-21 — تجديد ثانٍ يظهر «تم الحفظ» دون تحديث القائمة

- **المشكلة:** بعد تجديد أول ناجح، التجديد الثاني لـ `TC-07-2024-5019` / وحدة 88 يقول تم الحفظ لكن التواريخ ورقم العقد والشيكات لا تتحدث في شاشة العقود (تبقى `5019` / `2025-11-19`).
- **السبب:** (1) نواقص مستندات/دفتر عناوين كانت تحفظ **مسودة تجديد** فقط دون استبدال العقد المحفوظ؛ (2) دمج Neon `mergeContractPayloads` يفضّل العقد «الأغنى» بالمرفقات فيُبقي الاتفاق القديم عند المزامنة.
- **إصلاح:** زر «حفظ التجديد وتحديث العقد (المستندات لاحقاً)»؛ تفضيل بيانات التجديد الواردة محلياً وعلى الخادم؛ عرض مسودة التجديد في القائمة؛ ترقية المسودة العالقة `TC-07-2026-5028` في Neon؛ `BOOT_REV=2026-07-21-renewal-commit-update`.
- **تحقق:** Ctrl+F5 على شاشة العقود — يجب أن يظهر `TC-07-2026-5028` ونهاية `2026-02-19`.

### جلسة 2026-07-20 — 11 بوابة دفع خليجية (Kimi 04)

- **إضافة:** CMI، Network International، HyperPay، PayFort، MyFatoorah، PayTabs، Tap + تحديث المدير والـ PaymentSelector (فلاتر عمان/خليج).
- **مكان الظهور:** بوابة المستأجر `/portal/tenant/v2` فقط — ليس النظام القديم ولا `/admin` العام.
- **رفع المستندات:** لا تغيير في هذه الدفعة (المسارات الحالية كافية).
...
- **إصلاح:** ضمان مستخدم الجسر في السجل؛ عدم مسح جلسة الموقع فوراً؛ إبقاء بطاقات KPI ظاهرة؛ السماح بـ hub/تقويم أثناء ربط الموقع.

### جلسة 2026-07-19 — Overlay + كشف واحد بعد محاسبة/مهام (Kimi v6)

- **مراجعة:** `C:\Users\ahami\Downloads\Kimi_Agent_موقع العقارات` — تقرير الومضة + اقتراح Loading Overlay.
- **السبب المتبقي:** بطاقات hub (اعتماد/مهام) ومجاميع الإيجار كانت تُحمَّل بعد الكشف الأول.
- **إصلاح:** overlay بشريط تقدم؛ إخفاء `#module-body` حتى الجاهزية؛ سحب محاسبة/مهام/صيانة مع المفاتيح السريعة؛ `paintDashboardFirstReveal` يرسم جدول+hub+تقويم+شارة ثم يكشف؛ `LEGACY_SHELL_BOOT_REV=overlay-progress-v6`.
- **أرشيف:** تحديث `docs/reviews/kimi-agent-2026-07/`.

### جلسة 2026-07-19 — كشف واحد للوحة (بدون ومضة الشِل)

- **المشكلة:** أول ظهور ناقص (إنجليزي، إيجار 0، بدون hub/تقويم/أزرار) ثم بعد ~3ث الواجهة الصحيحة.
- **السبب:** الشِل كان يرسم ويكشف مبكراً ثم `initializeMode` يتخطى الرسم الكامل.
- **إصلاح:** إبقاء الـ overlay حتى `paintDashboardFirstReveal` (جدول+إحصائيات+hub) ثم إخفاء؛ هدوء 12ث يمنع إعادة الرسم الناعمة؛ `LEGACY_SHELL_BOOT_REV=single-reveal-v5`.
- **الملفات:** shell, app-main.js, route.ts

### جلسة 2026-07-19 — إصلاح ومضة 107 وحدة → 4 وحدات

- **السبب:** الرسم الأول من `managed_units` بدون دمج العقود؛ ثم `syncManagedUnitsFromProfiles` يمحو القائمة الكاملة ويستبدلها بناقص + عقود فقط.
- **إصلاح:** دمج العقود في المسار السريع والشِل؛ منع مزامنة profiles أثناء إقلاع اللوحة؛ حماية من تقليص القائمة؛ تأجيل إعادة البناء الكاملة مع هدوء 12ث.
- **الملفات:** app-main.js, shell, route.ts

### جلسة 2026-07-19 — استقرار بيانات اللوحة + أرشفة تقارير Kimi

- **مشكلة المستخدم:** تأخير ثم بيانات غير مستقرة تتغير فجأة (ومضة).
- **إصلاح:** انتظار جسر Neon (مفاتيح خفيفة) قبل الكشف؛ overlay حتى الرسم الأول؛ هدوء 4.5ث بدون إعادة رسم ناعمة؛ تخطي إعادة بناء الجدول إن رُسم من الشِل.
- **ملفات Kimi:** تقارير المراجعة نُسخت إلى `docs/reviews/kimi-agent-2026-07/` — لم يُستبدل `lib/encryption`/`lib/archive` الحاليان.
- **الملفات:** shell, bridge, app-main, route.ts, docs/reviews/...

### جلسة 2026-07-19 — منع تجميد اللوحة بعد الرسم المبكر

- **المشكلة:** البيانات تظهر ثم الصفحة تتوقف ولا تُقبل النقرات.
- **السبب:** بعد تحميل `app-main.js` كان `loadDashboardAux(fast)` يشغّل `syncManagedUnitsFromProfiles` + COA لكل وحدة، وparse لـ `building_profiles` الضخم على الخيط الرئيسي.
- **إصلاح:** تخطي profiles/COA في المسار السريع؛ تحميلها idle؛ تأجيل إعادة رسم الجدول؛ صفحة 50 صفاً؛ مهلة قصيرة بعد early-paint قبل تحميل السكربت.
- **الملفات:** `app-main.js`, `bhd-real-estate-shell.html`, `route.ts`
...
- **إصلاح:** رسم فوري من الكاش دون انتظار Neon؛ مزامنة خلفية؛ رسم اللوحة قبل `/api/health`؛ إحصاء المباني من `buildingsList` إن نقصت profiles؛ انتظار الجسر بدل فشل وإعادة المحاولة.
- **الملفات:** `legacy/.../js/app-main.js`

### جلسة 2026-07-16 — استعادة سرعة لوحة التحكم (كاش أولاً)

- **المشكلة:** جلب البيانات يستغرق أكثر من دقيقة بعد إجبار سحب Neon الكامل في كل فتح.
- **إصلاح:** إعادة تخطي السحب عند الكاش الدافئ؛ سحب مفاتيح خفيفة فقط عند الحاجة؛ المفاتيح الثقيلة (profiles/accounting) في الخلفية وعند النقص فقط؛ لا انتظار 6ث + force pull مزدوج.
- **الملفات:** `legacy/.../js/app-main.js`, `lib/server/legacyBridge.ts`

### جلسة 2026-07-16 — إصلاح عدم جلب بيانات اللوحة من Neon

- **المشكلة:** الصفحة تفتح لكن لا تظهر بيانات قاعدة البيانات.
- **الأسباب:** الجسر كان يتخطى السحب عند وجود كاش محلي؛ `__bhdBridgeStagedKvHydrated` يُضبط حتى عند الفشل فيمنع السحب اللاحق؛ `Promise.race` كان يعلّم الاكتمال قبل وصول البيانات.
- **إصلاح:** سحب Neon دائماً لوضع dashboard؛ تمييز `StagedKvHydratedOk`؛ سحب إجباري (`force`) إن لم ينجح الجسر؛ إعادة رسم إجبارية بعد السحب الناجح.
- **الملفات:** `legacy/.../js/app-main.js`, `lib/server/legacyBridge.ts`

### جلسة 2026-07-16 — إصلاح تعليق فتح لوحة التحكم بعد منع الوميض

- **المشكلة:** بعد إصلاح الوميض الصفحة لا تفتح (تبقى فارغة).
- **السبب:** انتظار تحميل Neon/الجسر قبل `initializeMode()` — إذا علّق طلب KV لا تُرسم الواجهة أبداً.
- **إصلاح:** إعادة الرسم الفوري من الكاش؛ مزامنة Neon في الخلفية بمهلة (Promise.race)؛ الإبقاء على بصمة البيانات لمنع إعادة الرسم المكررة.
- **الملفات:** `legacy/.../js/app-main.js`

### جلسة 2026-07-16 — إصلاح وميض بيانات لوحة التحكم (?mode=dashboard)

- **المشكلة:** شاشة فارغة → بيانات مؤقتة (localStorage) → ثم بيانات Neon الحقيقية خلال ثوانٍ.
- **الأسباب:** رسم سريع من الكاش المحلي؛ جسر KV يعيد `loadDashboardAux`؛ إقلاع Neon؛ idle maintenance؛ مزامنة تلقائية؛ تأجيل `syncManagedUnitsFromProfiles` بـ 800ms.
- **إصلاح:** بصمة بيانات + `requestDashboardRepaint` (لا إعادة رسم إن لم تتغير البصمة)؛ انتظار جسر Neon قبل أول رسم؛ مسار بارد يحمّل ثم يرسم مرة؛ الجسر يستدعي `__bhdRequestDashboardRepaint`؛ تأجيل auto-sync؛ مزامنة الوحدات فوراً في المسار السريع.
- **الملفات:** `legacy/.../js/app-main.js`, `lib/server/legacyBridge.ts`

### جلسة 2026-07-16 — تسريع وضع المحاسبة في النظام القديم

- **المشكلة:** `?mode=accounting` ينتظر Neon + `reconcile=1` قبل أي رسم، ويسحب كل مفاتيح `bhd_*` عند الإقلاع البارد.
- **إصلاح:** رسم فوري من localStorage إن وُجدت بيانات؛ مزامنة Neon في الخلفية بدون reconcile؛ مفاتيح محاسبية انتقائية فقط؛ staged hydrate مبكر في bridge للمحاسبة.
- **الملفات:** `legacy/.../js/app-main.js`, `lib/server/legacyBridge.ts`

### جلسة 2026-07-16 — إصلاح صفحة تصفير البيانات /admin/data

- **سبب العطل المحتمل:** رمز الحماية غير مُعدّ (`ADMIN_DATA_RESET_PIN`) → فشل صامت/غامض؛ رسائل خطأ غير واضحة؛ الصفحة تعلق على التحميل بدون حالة جلسة.
- **إصلاح:** `GET /api/admin/data/pin-status`؛ أخطاء `PIN_NOT_CONFIGURED`/`INVALID_PIN` واضحة؛ التصفير لا يفشل إن فشل زرع PIN بعد المسح؛ بانر حالة الرمز في الواجهة؛ تحسين تحميل صلاحية ADMIN.
- **مطلوب في Vercel:** `ADMIN_DATA_RESET_PIN` (8+ أحرف) إن لم يكن الرمز موجوداً في DB.
...
- **Modals:** `AccountingHubModals.tsx` — تجميع 6 modals + طباعة
- **Unit test:** `buildAccountingHubPath.test.ts` — 24 اختباراً إجمالاً
- **`AccountingSection.tsx`:** ~430 سطر (كان ~580)
- **`npm run build`** ✅ + **`npm run test:unit`** ✅ — **رفع Git**
...
- **التقارير:** منشئ CSV مخصص + جدولة تقارير (`/admin/reports`) — `report-schedules` API
- **`loading.tsx`:** analytics، reports، site، document-templates
- **Push:** `26de3f6` (phase 5) + commit جديد
...
- **ترحيل الحجوزات القديمة:** `lib/data/migrateBookingIdentityDocs.ts` + `ensureBookingDocumentsHydrated` في `bookingDocuments.ts`؛ زر «طلبات هوية للحجوزات الحالية» في `/admin/bookings` يملأ الطلبات الناقصة دون حذف المرفوع.
- **ملفات:** `book/page.tsx`, `bookingTerms.ts` (`getPreBookingIdentityRequirementsPersonal`), `bookings.ts` (`contractKind`, `contactNationality`), `addressBook.ts` (`ensureContactFromBooking` + `nationality`), `BookingDocumentsPanel.tsx`, `admin/bookings/page.tsx`, `migrateBookingIdentityDocs.ts`.
...
- **الإصلاح:** إن وُجدت مستندات بانتظار التأكيد أو لم تُعتمد المطلوبة أو الشيكات غير مكتملة → `upload-documents`؛ وإلا → `contract-review`. زر اعتماد المالك يبقى على `contract-review`. `encodeURIComponent` لـ `bookingId` في الروابط.

### جلسة 2026-04-02 — المستخدمون (/admin/users): تصميم مضغوط مثل دفتر العناوين

- **التغيير:** `users-admin-page-compact` + رأس `compact`، بطاقات إحصاء أصغر، شريط بحث/دور، قائمة بطاقات مستخدم بخط أصغر وباركود 24px وأزرار `text-[11px]`؛ أنماط في `admin.css` لحشو عناصر القائمة.

### جلسة 2026-04-02 — دفتر العناوين: إعادة تصميم مضغوطة

- **التغيير:** خط أصغر للصفحة، تباعد أوضح (`space-y-4`)، بطاقات إحصاء أصغر، شريط فلاتر بخلفية خفيفة، جدول بعمود رأس مضغوط (`address-book-table--compact`)، أزرار رأس الصفحة `text-xs`؛ `AdminPageHeader` يدعم `compact` و`actionsClassName`.

### جلسة 2026-04-02 — لوحة العميل + «حسابي»: بطء حتى تظهر البيانات

- **السبب:** `GET /api/me/accounting-documents` كان يحمّل **كل** مستندات النوع من قاعدة البيانات ثم يفلتر في الذاكرة؛ وصفحة «حسابي» كانت تشغّل **بالتوازي** `linked-contact` و`/api/subscriptions/me`؛ لوحة العميل كانت تجلب **بالتوازي** الحجوزات وطلبي محاسبة.
- **الإصلاح:** `getDocumentsFromDb({ contactId })` + فهرس `@@index([contactId])` وترحيل SQL؛ تأجيل اشتراك المستخدم حتى بعد اكتمال محاولة `linked-contact`؛ تسلسل جلب لوحة العميل: حجوزات ثم أعداد الإيصال/الفاتورة.

### جلسة 2026-04-07 — دفتر العناوين: بطء 10s+ حتى الأسماء

- **السبب:** ثلاثة طلبات ثقيلة بالتوازي عند فتح الصفحة (`/api/address-book` + `/api/bookings` + مستندات محاسبة limit=1000)، وانتظار `getSession()` قبل الطلب، وإعادة محاولة 5xx خمس مرات مع تأخيرات طويلة.
- **الإصلاح:** تأجيل حجوزات + مستندات حتى `!contactsLoading`؛ تقليل limit المستندات إلى 400؛ عدم انتظار `getSession`؛ تقليل محاولات 5xx إلى مرتين وتقصير التأخير؛ `cache: 'no-store'` على الطلبات الثانوية.

### جلسة 2026-04-07 — بطء لوحة التحكم (متابعة)

- **السبب إضافي:** في `GET /api/bookings` كان يُولَّد رقم BKG ويُحدَّث الصف في **نفس الطلب** لكل حجز ناقص — استعلامات متسلسلة (ثوانٍ مع عشرات السجلات).
- **الإصلاح:** جمع مهام التوليد في `after()`؛ الاستجابة تعيد البيانات فوراً والإصلاح يتم في الخلفية. طلب الحجوزات من لوحة التحكم: `cache: 'no-store'`.
